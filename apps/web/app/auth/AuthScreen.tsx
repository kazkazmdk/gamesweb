...
import { resetTurnstile, TurnstileWidget } from "@/components/auth/TurnstileWidget";
...
export function AuthScreen({ turnstileNonce, turnstileSiteKey }: { turnstileNonce: string; turnstileSiteKey: string }) {
  useAccent();
  ...
  const canSubmit = configured && !busy && (!captchaRequired || Boolean(captchaToken));
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (captchaRequired && !captchaToken) {
      setError("Confirm you are human, then try again.");
      return;
    }
    setBusy(true);
    setError("");
    ...
    <div className="mx-auto max-w-md px-5 py-16">
      ...
      {sent ? (
        ...
        <form onSubmit={onSubmit} className="mt-8 space-y-4">
          ...
          {captchaRequired ? <TurnstileWidget siteKey={turnstileSiteKey} nonce={turnstileNonce} onToken={setCaptchaToken} /> : null}
          <button type="submit" className="h-12 w-full rounded-full bg-[var(--text)] text-[var(--bg)]" disabled={!canSubmit}>
            {busy ? "Checking…" : configured ? "Continue" : "Unavailable in local mode"}
          </button>
